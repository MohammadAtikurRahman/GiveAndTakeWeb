...
$searchquery="SELECT * FROM advertiesment WHERE exchangePlace LIKE '%$searchval%'";
try{
    ...
    if($object->rowCount() == 0){
        ...
        foreach($table as $row){
            $tablecode.="<tr><td>$row[0]</td><td>$row[2]</td><td>$row[3]</td><td>$row[5]</td><td>$row[6]</td><td>$row[7]</td></tr>";
        }
